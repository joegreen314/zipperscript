...
4.0.57	Fixed bug where zipperscript would crash while zipping validation routes
		To speed up script, log display updates no more than once/second and FIS files are not compressed (they are already compressed)
		Start and browse buttons deactivate once script starts
		Log file now breaks down file sizes by data type and gives a % compressed
		Adds all files written within last 24 hours from "Desktop/DVX Pictures" and "Desktop/Error_Archive" to collection zip
...
Output files:
The zipper script will make up to three zip files in the same directory as the root directory selected.  It will also make a log file that should be useful for troubleshooting purposes.  The logfile is named zipperscript_log_VEHICLE_DATETIME.txt and should be uploaded along with the zip files.  All files are compressed except FIS files, because they are already compressed and attempting to compress these files further takes a long time.
...
1)  DATE_VEHICLE_Collection.zip
	...
	All files written within last 24 hours from "Desktop/DVX Pictures" and "Desktop/Error_Archive" (ignoring files larger than 50 MB)
	From routes folders:
		.txt
		.pdf
		.rut
		...
		Three FIS file from three routes, ignoring routes with “burn” or “bounce” in the name
